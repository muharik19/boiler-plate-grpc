...
import (
	...
	"github.com/muharik19/boiler-plate-grpc/pkg/utils"
...
func Configure() {
	switch *utils.Getenv("LOG_FORMAT") {
	case "json":
	...
	lvl := strings.ToLower(*utils.Getenv("LOG_LEVEL"))
	switch lvl {
	case "debug":
...
func GetColorCode(c Color, t ColorType) (string, bool) {
	defaultCode := []string{"39", "49"}
	blackCode := []string{"30", "40"}
	redCode := []string{"31", "41"}
	greenCode := []string{"32", "42"}
	yellowCode := []string{"33", "43"}
	blueCode := []string{"34", "44"}
	magentaCode := []string{"35", "45"}
	cyanCode := []string{"36", "46"}
	lightGrayCode := []string{"37", "47"}
	darkGrayCode := []string{"90", "100"}
	lightRedCode := []string{"91", "101"}
	lightGreenCode := []string{"92", "102"}
	lightYellowCode := []string{"93", "103"}
	lightBlueCode := []string{"94", "104"}
	lightMagentaCode := []string{"95", "105"}
	lightCyanCode := []string{"96", "106"}
	whiteCode := []string{"97", "107"}
	...
	fcs := map[Color][]string{
		...
		MAGENTA:       magentaCode,
		CYAN:          cyanCode,
		LIGHT_GRAY:    lightGrayCode,
		DARK_GRAY:     darkGrayCode,
		LIGHT_RED:     lightRedCode,
		LIGHT_GREEN:   lightGreenCode,
		LIGHT_YELLOW:  lightYellowCode,
		LIGHT_BLUE:    lightBlueCode,
		LIGHT_MAGENTA: lightMagentaCode,
		LIGHT_CYAN:    lightCyanCode,
		WHITE:         whiteCode,
	}
